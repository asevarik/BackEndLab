...
import { userLoginObject, userObject } from "../Modals/zod/User";
...
export const sanatizeUserSignInModelMiddleware = async (req:Request, res:Response, next:NextFunction) => {
    const user = req.body;
    try{
        const isUserValid = await userLoginObject.safeParseAsync(user);
        if(!isUserValid.success){
            const erros = await isUserValid.error.flatten().fieldErrors;
            //If the user has not provided in the body or anything
            if(Object.keys(erros).length === 0){
